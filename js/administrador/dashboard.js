...
function initializeUserSection() {
    document.getElementById('total-users').textContent = mockData.users.total.toLocaleString('pt-BR');
    document.getElementById('new-users').textContent = mockData.users.new.toLocaleString('pt-BR');
...
// Inicializa os filtros de tempo
function initializeTimeFilters() {
    const userTimeFilter = document.getElementById('user-time-filter');
    const investmentTimeFilter = document.getElementById('investment-time-filter');

    if (userTimeFilter) {
        userTimeFilter.addEventListener('change', function () {
            // Aqui você pode adicionar a lógica para atualizar os dados baseado no período selecionado
            console.log('Período de usuários alterado para:', this.value);
        });
    }

    if (investmentTimeFilter) {
        investmentTimeFilter.addEventListener('change', function () {
            // Aqui você pode adicionar a lógica para atualizar os dados baseado no período selecionado
            console.log('Período de investimentos alterado para:', this.value);
        });
    }
}
