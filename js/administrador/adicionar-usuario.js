document.addEventListener('DOMContentLoaded', function() {
        ...
        try {
            ...
            if (!response.ok) {
                if (responseData.errors && responseData.errors.length > 0) {
                    throw new Error(responseData.errors[0] || responseData.msg || "Erro ao adicionar usuário");
                }
    ...
    userForm.addEventListener("submit", async function(event) {
        ...
        if (resultado && resultado.data) {
            mostrarMensagem(`Usuário ${perfilMapeado.toLowerCase()} adicionado com sucesso!`, "sucesso");
            setTimeout(() => {
                this.reset();
            }, 2000);
        }
