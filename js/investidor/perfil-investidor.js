...
let etapaAtual = 0, pontuacao = 0, respostaSelecionada = -1;
...
function atualizarBarraProgresso() {
    if (etapaAtual === 0) {
        ...
        barraProgresso.style.width = etapaAtual >= perguntas.length ? "100%" : `${(etapaAtual / (perguntas.length - 1)) * 100}%`;
        textoProgresso.innerText = etapaAtual >= perguntas.length ? "Concluído" : `Questão ${etapaAtual} de ${perguntas.length - 1}`;
    }
}
        ...
        if (pergunta.tipo === "info") {
            ...
            div.querySelectorAll('.botao-opcao').forEach(btn => {
                btn.addEventListener('click', () => {
                    div.querySelectorAll('.botao-opcao').forEach(b => b.classList.remove('selecionado'));
...
function voltarEtapa() {
    if (etapaAtual > 0) {
        if (respostasUsuario[etapaAtual] !== -1) pontuacao -= respostasUsuario[etapaAtual];
        etapaAtual--;
...
function mostrarResultado() {
    // Calcula perfil baseado na pontuação
    const perfil = pontuacao <= 40 * 0.33 ? "CONSERVADOR" : pontuacao <= 40 * 0.66 ? "MODERADO" : "ARROJADO";
    ...
    // Atualiza perfil e exibe resultado
        ...
        .then(sucesso => {
            etapasContainer.innerHTML = `
                <div class="resultado">
                    ...
                    ${sucesso ? '<p class="sucesso">Seu perfil foi salvo com sucesso!</p>' : '<p class="aviso">Não foi possível salvar seu perfil no servidor. Suas preferências serão mantidas apenas localmente.</p>'}
...
btnPular.addEventListener("click", () => { etapaAtual = perguntas.length; mostrarResultado(); });
